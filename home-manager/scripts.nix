...
in {
  home.packages = [
    ...
    (mkScript "refresh-waybar" { file = ../bin/refresh-waybar; runtimeInputs = [ pkgs.psmisc pkgs.waybar ]; })
    (mkScript "refresh-wm" { file = ../bin/refresh-wm; runtimeInputs = [ pkgs.hyprland pkgs.sway ]; })
    ...
    (mkScript "theme" { file = ../bin/theme; runtimeInputs = [ pkgs.alacritty pkgs.fuzzel pkgs.gnused pkgs.mako pkgs.procps pkgs.swaybg pkgs.waybar ]; })
